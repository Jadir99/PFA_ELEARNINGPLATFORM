...
function lsk() {
  ...
  const welcomeMessages = [
    "The service is currently unavailable. The final version will be released soon. We look forward to your visit in the near future.<br/>. الخدمة متوقفة حاليا، النسخة النهائية ستصدر قريبا، نتمنى زيارتكم عما قريب "
  ];

  const userMessage = txtarea.value.trim();
  if (userMessage === '') return;

  // Remove any previous loading message if present
  ...
  // Set a timer to remove the loading message after 2 seconds
  setTimeout(() => {
    loadingMessage.remove();

    // Add the user's message
    const userMessageElement = createMessageElement('You', userMessage, 'user-speech', 'assets/images/avatar-homme.png', true);
    appendMessage(userMessageElement);

      // Display a welcome message or auto-reply for other messages
      const defaultMessage = welcomeMessageIndex < welcomeMessages.length
        ? welcomeMessages[welcomeMessageIndex++]
        : generateAutoReply();
      responseElement = createMessageElement('AZUL AI', defaultMessage, 'ai-speech', 'assets/images/azul-icon.png', false);
    // }

    appendMessage(responseElement);
    txtarea.value = ''; // Clear the text area
  }, 2000); // 2-second delay
}
...
function appendMessage(element) {
  ...
  function saveAndRegenerateMessage(button) {
    ...
    const regeneratedMessageElement = createMessageWithReactions('ChatenAI', regeneratedMessage, 'ai-speech', 'assets/images/avatar-homme.png');
