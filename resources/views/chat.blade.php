...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('public/assets/css/bundle0ae1.css?v1310') }}">
    <link rel="stylesheet" href="{{ asset('public/assets/css/app0ae1.css?v1310') }}">
...
<body class="tyn-body">
    ...
    <div class="tyn-root">
        ...
        <div class="tyn-content tyn-content-full-height tyn-chatbot tyn-chatbot-page has-aside-base">
            <div class="tyn-aside tyn-aside-base">
                <!-- ... (tyn-aside content remains the same) ... -->
                ...
                <div class="tyn-chat-body my-4 px-4 js-scroll-to-end" id="tynBotBody">
                    <div class="container px-0">
                        <div class="tyn-qa tyn-qa-bubbly" id="tynBotReply">
                            <!-- Messages generated by JavaScript will be appended here -->
                        </div>
                    </div>
                </div>
                    ...
                    <div class="container px-0">
                        <div class="ps-3 pe-4 py-3 bg-white mb-4 rounded-3">
                            <div class="tyn-chat-form-enter">
                                <div class="tyn-chat-form-input" id="tynBotInput" placeholder="Type your message..." contenteditable>
                                </div>
        ...
        <div class="tyn-quick-chat" id="tynQuickChat">
            <!-- ... (tyn-quick-chat content remains the same) ... -->
        </div>
    </div>

    <script>
        function initializeChat() {
    const chatBox = document.getElementById('tynBotReply');
    if (!chatBox) {
        console.error('Chat container #tynBotReply not found! Aborting chat initialization.');
        return;
    }
    chatBox.innerHTML = '';

    const userInput = document.getElementById('tynBotInput');
    if (!userInput) {
        console.error('User input #tynBotInput not found! Aborting chat initialization.');
        return;
    }
    const sendButton = document.getElementById('tynBotSend');
    if (!sendButton) {
        console.error('Send button #tynBotSend not found! Aborting chat initialization.');
        return;
    }
    const chatBodyScroller = document.getElementById('tynBotBody');
    if (!chatBodyScroller) {
        console.warn('Chat body scroller #tynBotBody not found! Auto-scrolling may not work as expected.');
    }

    // const user_id = 651651;
    const user_id =  @json(currentUserId());;
    const welcomeMessages = ["Hello, how can I help you?"];

    const userAvatarSrc = 'images/avatar/1.jpg';
    const aiAvatarSrc = 'images/avatar/bot-1.jpg';

    let isSending = false; // --- SOLUTION PART 2: Flag to prevent duplicate sends ---

    function addMessage(content, senderType, imgSrc) {
        const qaItemDiv = document.createElement('div');
        qaItemDiv.className = 'tyn-qa-item';

        const avatarContainerDiv = document.createElement('div');
        avatarContainerDiv.className = 'tyn-qa-avatar';
        const mediaDiv = document.createElement('div');
        mediaDiv.className = 'tyn-media tyn-size-md';
        const img = document.createElement('img');
        img.src = imgSrc;
        img.alt = senderType === 'user' ? 'User Avatar' : 'AI Avatar';
        mediaDiv.appendChild(img);

        if (senderType === 'ai') {
            const avatarWrapDiv = document.createElement('div');
            avatarWrapDiv.className = 'tyn-qa-avatar-wrap';
            avatarWrapDiv.appendChild(mediaDiv);
            avatarContainerDiv.appendChild(avatarWrapDiv);
        } else {
            avatarContainerDiv.appendChild(mediaDiv);
        }
        qaItemDiv.appendChild(avatarContainerDiv);

        const messageContentDiv = document.createElement('div');
        messageContentDiv.className = 'tyn-qa-message tyn-text-block';
        messageContentDiv.innerHTML = content;
        qaItemDiv.appendChild(messageContentDiv);

        chatBox.appendChild(qaItemDiv);
        const scroller = chatBodyScroller || chatBox;
    ...
    function showLoadingMessage() {
        ...
        const qaItemDiv = document.createElement('div');
        qaItemDiv.className = 'tyn-qa-item generate-section';
        const loadingImgSrc = "https://www.icegif.com/wp-content/uploads/2022/12/icegif-566.gif";
        ...
        const messageContentDiv = document.createElement('div');
        messageContentDiv.className = 'tyn-qa-message tyn-text-block';
        const loadingTitle = document.createElement('h6');
        loadingTitle.className = 'title color-text-off mb--0';
        loadingTitle.textContent = 'Generating answers for you…';
        messageContentDiv.appendChild(loadingTitle);
        qaItemDiv.appendChild(messageContentDiv);
    ...
    function removeLoadingMessage() {
        const previousLoadingMessage = chatBox.querySelector('.generate-section');
    ...
    sendButton.addEventListener('click', () => {
        if (isSending) { // --- SOLUTION PART 2: Check flag ---
        ...
        isSending = true; // --- SOLUTION PART 2: Set flag ---
        sendButton.disabled = true; // Disable button to prevent rapid clicks
        userInput.setAttribute('contenteditable', 'false'); // Optionally disable input editing

        addMessage(message, 'user', userAvatarSrc);
        userInput.textContent = ''; // Clear input AFTER getting message and BEFORE fetch
        showLoadingMessage();
        ...
        .then(data => {
            console.log("AI Response Data:", data); // For debugging server response
            removeLoadingMessage();
            const rawResponseText = data && data.response ? String(data.response) : ''; // Ensure it's a string

            // Trim the raw response string itself. If it becomes empty, it's definitely empty.
            const trimmedResponseText = rawResponseText.trim();

            if (trimmedResponseText === '') {
                console.warn("Received AI response that is an empty string or only whitespace. Not displaying. Raw response:", rawResponseText);
            } else {
                // If not an empty string, then check its rendered content
                const tempDiv = document.createElement('div');
                tempDiv.innerHTML = trimmedResponseText; // Use the already trimmed HTML string
                const plainTextContent = (tempDiv.textContent || tempDiv.innerText || "").trim();
                // Check for specific content-bearing elements in addition to plain text
                const hasMeaningfulElements = tempDiv.querySelector('img, iframe, video, canvas, svg, pre, code, ul, ol, table, blockquote, h1, h2, h3, h4, h5, h6, p:not(:empty)');


                if (plainTextContent || hasMeaningfulElements) {
                    addMessage(trimmedResponseText, 'ai', aiAvatarSrc); // Add the message
                } else {
                    console.warn("Received AI response that renders as empty. Not displaying. Raw response:", rawResponseText, "Rendered text:", plainTextContent);
                }
            }
        })
        ...
        .catch(error => {
            ...
            removeLoadingMessage();
            addMessage(`Sorry, something went wrong with the connection: ${error.message}. Please try again.`, 'ai', aiAvatarSrc);
        })
        .finally(() => { // --- SOLUTION PART 2: Reset flag and button ---
            isSending = false;
            sendButton.disabled = false;
            userInput.setAttribute('contenteditable', 'true'); // Re-enable input
        });
    });
    ...
    userInput.addEventListener('keydown', (event) => {
        if (event.key === 'Enter' && !event.shiftKey) {
            ...
            if (!isSending) { // Only trigger click if not already sending
                sendButton.click();
            }
        }
    });

    addMessage(welcomeMessages[0], 'ai', aiAvatarSrc);
}

window.onload = initializeChat;
    </script>

    <script src="{{ asset('public/assets/js/bundle0ae1.js?v1310') }}"></script>
    <script src="{{ asset('public/assets/js/app0ae1.js?v1310') }}"></script>
